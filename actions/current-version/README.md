...
A GitHub Action that extracts the current Cargo workspace version using `cargo-goose` and exposes it as structured outputs.
...
```bash
cargo goose current-version --format=json
```
...
* Version parsing logic lives entirely in `cargo-goose`; this action does not reimplement semver logic.
